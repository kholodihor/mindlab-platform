...
const TopicItem = ({
  ...
  return (
    <li
      className={` ${activeTopic === number ? "bg-darkGray 3md:border-l-4 3md:border-lightGray 3md:border-l-violet 3md:bg-inherit" : ""} 3md-border cursor-pointer border-t border-darkGray py-5 pl-8 pr-5 3md:w-[100%] 3md:rounded-md 3md:border-[0.5px]`}
      onClick={() => onClickLesson(number)}
    >
      <div className="mb-3 flex justify-between ">
        <p className="w-[90%] sm:text-lg">
          <span className="text-lightViolet">
            {duration ? `${t("lesson")} ${number}.` : `Lesson ${number}.`}
          </span>{" "}
          {title}
        </p>
        <div
      ...
      <div className="flex gap-1 ">
        <p className={`${duration ? "" : "underline"}  text-sm text-gray`}>
          {duration
